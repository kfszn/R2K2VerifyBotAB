...
try {
await pool.query(`CREATE TABLE IF NOT EXISTS rewards ( id SERIAL PRIMARY KEY, username VARCHAR(255) NOT NULL, reward_type VARCHAR(50) NOT NULL, amount DECIMAL(10, 2) NOT NULL, period INTEGER NOT NULL, net_loss DECIMAL(10, 2), claimed_by VARCHAR(255) NOT NULL, timestamp TIMESTAMP DEFAULT CURRENT_TIMESTAMP, site VARCHAR(50) DEFAULT 'acebet' )`);
...
try {
await pool.query(
`INSERT INTO user_links (discord_id, acebet_username) VALUES ($1, $2) ON CONFLICT (discord_id) DO UPDATE SET acebet_username = $2`,
...
const query = `INSERT INTO rewards (username, reward_type, amount, period, net_loss, claimed_by, timestamp, site) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *`;
const values = [
reward.username,
reward.reward_type,
reward.amount,
...
new SlashCommandBuilder()
.setName(‘payouts’)
.setDescription(‘Detailed payout breakdown for a period (Owner only)’)
.addIntegerOption(option => option.setName(‘period’).setDescription(‘Period number (1-12)’).setRequired(true).setMinValue(1).setMaxValue(12)),
...
const links = await loadLinks();
if (links[discordId]) { await interaction.editReply(`❌ You are already linked to **${links[discordId]}**. Use \`/unlink` first.`); return; } await saveLink(discordId, user.name); await interaction.editReply(`✅ Successfully linked your Discord account to Acebet username **${user.name}**`);
} catch (error) {
...
// ─────────────────────────────────────────────
// /payouts [period] — Detailed period breakdown
// ─────────────────────────────────────────────
...
try {
  ...
  const result = await pool.query(
    'SELECT * FROM rewards WHERE period = $1 ORDER BY username ASC, timestamp ASC',
    [period]
  );
  const claims = result.rows;

  if (claims.length === 0) {
    await interaction.editReply(`📊 No payouts recorded for Period ${period} (${startDateStr} – ${endDateStr})`);
    return;
  }

  const rewardTypeNames = { lossback: 'Lossback', wagerbonus: 'Wager Bonus', depobonus: 'Deposit Bonus', gw: 'Giveaway' };
  const siteEmoji = { acebet: '🔵', packdraw: '🟣' };
  ...
  for (const [type, total] of Object.entries(categoryTotals).sort((a, b) => b[1] - a[1])) {
    ...
    msg += `• ${rewardTypeNames[type] || type}: **$${total.toFixed(2)}** (${count} claim${count !== 1 ? 's' : ''})\n`;
  }
  msg += `\n`;

  // Per-user breakdown
  msg += `👤 **Per User** (${uniqueUsers} user${uniqueUsers !== 1 ? 's' : ''})\n`;
  msg += `━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  ...
  for (const [user, claimList] of Object.entries(userClaims).sort((a, b) => userTotals[b[0]] - userTotals[a[0]])) {
    ...
    for (const c of claimList) {
      const date = new Date(c.timestamp).toLocaleString('en-US', {
        month: 'short', day: 'numeric',
        hour: 'numeric', minute: '2-digit', hour12: true, timeZone: 'America/New_York'
      });
      const site = siteEmoji[c.site] || '⚪';
      const netNote = c.net_loss ? ` (net loss: $${parseFloat(c.net_loss).toFixed(2)})` : '';
      msg += `  ${site} ${rewardTypeNames[c.reward_type] || c.reward_type}: $${parseFloat(c.amount).toFixed(2)}${netNote} — ${date} EST\n`;
    }
  }

  msg += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  msg += `💰 **Grand Total Paid: $${grandTotal.toFixed(2)}** across ${claims.length} claim${claims.length !== 1 ? 's' : ''}`;
